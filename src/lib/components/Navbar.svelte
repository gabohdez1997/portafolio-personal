...
<nav
	class="fixed top-0 left-0 z-50 flex w-full items-center justify-between border-b border-white/5 bg-black/30 px-6 py-4 backdrop-blur-md"
>
	...
	<button
		onclick={toggleMenu}
		class="relative z-[60] p-2 text-white md:hidden"
	...
	{#if isOpen}
		<div
			...
			class="fixed inset-0 z-50 flex h-screen w-screen flex-col items-center justify-center bg-black/95 backdrop-blur-xl md:hidden"
			onclick={closeMenu}
			aria-hidden="true"
		>
			<ul
